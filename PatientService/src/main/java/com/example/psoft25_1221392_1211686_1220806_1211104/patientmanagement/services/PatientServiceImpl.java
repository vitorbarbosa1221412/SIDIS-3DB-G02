...
public class PatientServiceImpl implements PatientService {
	...
	public ResponseEntity<Patient> getPatientById(Long id) {
		...
		if (patient == null) {
			// Forward GET to peers and, if found, return it
			Optional<Patient> fromPeer = integrationService.getFromPeers("/api/patients/id/" + id + "/profile", Patient.class);
			return fromPeer
	...
	public List<Patient> searchByPatientName(String name, Page page) {
		...
		if (patients.isEmpty()) {
			// Forward GET to peers: expect an array payload from peer endpoint
			Optional<Patient[]> fromPeer = integrationService.getFromPeers("/api/patients/name/" + name + "/profile", Patient[].class);
			return fromPeer.map(arr -> Arrays.asList(arr)).orElseThrow(() -> new NotFoundException("Patient with name " + name + " not found"));
		}
		return patients;
	}
